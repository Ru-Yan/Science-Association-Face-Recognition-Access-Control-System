...
void compare_face(vector<float>& know_face, vector<float>& face) {
    vector<float>& dis = face_distance(know_face, face);
}
...
void MTCNN::GetSamples(string label,cv::Mat img){
    ...
    while (!img.empty())
    {
        ...
        if (finalBbox.size()>0)
        {
            cout << "检测到人脸个数：" << finalBbox.size() << endl;
            ...
            Mat myFace;
            QString time1 = QDateTime::currentDateTime().toString("yyyy-MM-dd");
            QString time2 = QDateTime::currentDateTime().toString("hh-mm-ss");

            string s_time1 = time1.toStdString();
            string s_time2 = time2.toStdString();
            string s_time = s_time1 + "-" + s_time2;
            if (finalBbox.size() == 1)       //有一个人脸时可以拍照
            {
                //cv::imwrite("./data/" + label + "-" + s_time + ".jpg", img);
                string save_path_img = "./data/" + label + "-" + s_time + ".jpg";
                cv::imwrite(save_path_img, img);
                cout << save_path_img<<endl;
                break;
            }
            else cout << "屏幕中只有一个人脸时才能拍照哦！" << endl;
        }
    }
}

vector<BoxInfo> MTCNN::decode_infer(ncnn::Mat& data, const cv::Size& frame_size, float threshold) {
...
void MTCNN::g_r(std::vector<BoxInfo>& input_boxes, float NMS_THRESH) {
